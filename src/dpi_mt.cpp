...
struct Stats {
    ...
    std::atomic<uint64_t> https_packets{0};
...
public:
    ...
    bool process(const std::string& input_file, const std::string& output_file) {
        ...
        while (reader.readNextPacket(raw)) {
            ...
            if (parsed.has_tcp) {
                stats_.tcp_packets++;
                if (parsed.dest_port == 443) stats_.https_packets++;
            } else if (parsed.has_udp) {
                stats_.udp_packets++;
            }
...
private:
    ...
    void printReport() {
        ...
        // Clean summary
        auto blocked_domains = rules_.getBlockedDomains();
        std::cout << "\n==============================\n";
        std::cout << "  Output Summary\n";
        std::cout << "==============================\n";
        std::cout << "  Total packets: " << stats_.total_packets.load() << "\n";
        std::cout << "  TCP:           " << stats_.tcp_packets.load() << "\n";
        std::cout << "  HTTPS:         " << stats_.https_packets.load() << "\n";
        if (!blocked_domains.empty()) {
            std::string list;
            for (size_t i = 0; i < blocked_domains.size(); i++) {
                if (i > 0) list += ", ";
                list += blocked_domains[i];
            }
            std::cout << "  Blocked domains: " << list << "\n";
        }
        std::cout << "==============================\n";
    }
};
